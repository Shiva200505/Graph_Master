...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // Hide customer nav/footer on admin and dealer portals
  const headersList = await headers();
  const pathname = headersList.get('x-pathname') ?? '';
  const isPortal = pathname.startsWith('/admin') || pathname.startsWith('/dealer');
  ...
  return (
    <html lang="en">
      ...
      <body>
        {!isPortal && <Navbar />}
        <main style={!isPortal ? { minHeight: 'calc(100vh - 64px)' } : undefined}>
          {children}
        </main>
        {!isPortal && (
